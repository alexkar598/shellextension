...
pub trait BoundedOptionalDeref<T, R> {
    fn with_optional_ref(
        self,
        f: impl FnOnce(Option<&T>) -> windows_core::Result<R>,
...
impl<T, R> BoundedOptionalDeref<T, R> for *const T {
    fn with_optional_ref(
        self,
        f: impl FnOnce(Option<&T>) -> windows_core::Result<R>,
    ) -> windows_core::Result<R> {
        let ptr = unsafe { self.as_ref() };
        f(ptr)
    }
}
